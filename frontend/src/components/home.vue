<template #default>
  <div id="homeMain" ref="home">
    <n-row class="home-header">
      <n-col :span="4"></n-col>
      <n-col style="margin-top: 15px;text-align: left;" :span="3">
        <svg viewBox="0 0 170 35" focusable="false" class="chakra-icon md:!w-[170px] md:!h-[32px] !w-[89px] !h-[42px] cursor-pointer css-on
          ...
          <path fill="white" d="M57.61 17.92c.15 0 .35-.05.51-.05 6.13-.56 10-4.19 10-8.63s-3.78-8-9.86-8.64c-.56 0-1.18-.1-1.79-.1h-47
					...
					5.11 2 5.11 4.44s-1.58 4.35-5.11 4.45H101c-5.73 0-5.27-3.73-5.27-3.73V9.8s-.42-3.53 5.36-3.53z"
            transform="translate(-1.5 -.5)"></path>
        </svg>
      </n-col>
      <n-col :span="11"></n-col>
      <n-col :span="2" style="text-align: left;">
        <div style="margin-top: 15px;font-size: 18px;" id="header-login" @click="goEvent('name')"
          v-if="userInfo.userMessage.isLogin">{{ userInfo.userMessage.name }}</div>
        <div style="margin-top: 15px;font-size: 18px;" id="header-login" @click="goEvent('log')" v-else>{{
        t('commons.login') }}</div>
      </n-col>
      <n-col :span="2" style="width: 80px;text-align: left;">
        <div style="margin-top: 15px;font-size: 18px;" id="header-register">
          <span class="header-sign-up" @click="goEvent('logout')" v-if="userInfo.userMessage.isLogin">{{
        t('commons.logout') }}</span>
    ...
    <n-row>
      ...
      <n-col :span="18">
        <div style="text-align: center;margin-top: 100px;font-size: 35px;font-weight: bold;">
          ...
          <span style="font-size: 22px;"><b>A</b>ritificial <b>I</b>ntelligence <b>G</b>enome
            <b>I</b>nterpretation</span>
        </div>
        <div style="font-size: 18px;padding-bottom: 20px;">{{ t('modules.desc4') }}</div>
        <n-auto-complete v-model:value="searchInput" @click="search" @select="handleSelection" @keyup.enter="search"
          :options="searchOptions" :placeholder="$t('commons.searchPlaceholder')" id="search-input" size="large"
          clearable>
          <template #suffix>
            <div style="display:flex; justify-content: space-between;">
              <n-select class="ref-selection" :placeholder="$t('commons.inputPlaceholder')" v-model:value="refValue"
                :options="refOptions" />
              <n-select class="input-selection" :placeholder="$t('commons.inputPlaceholder')"
                v-model:value="selectValue" :options="options" />
            </div>
